...
void change_security_question(user * u)//39
{
    print_basic_screen();
    FILE * file_user = fopen("C:\\Movie Ticket Booking\\Users.bin", "rb+");
    if(file_user == NULL)
    {
        print_char(' ', 79, 25, BLACK);
        print_title("Error in opening file \"Users.bin\" change_security_question(user * u)", 1, 25, RED);
        perror("Error");
        getch();
        return;
    }
    ...
    if(choice == 'y' || choice == 'Y')
    {
        ...
        while(fread(&us, sizeof(us), 1, file_user) == 1)
        {
            if(!strcmp(u->userid, us.userid))
            {
                fseek(file_user, -(long long signed int)sizeof(us), SEEK_CUR);
                fwrite(u, sizeof(*u), 1, file_user);
                break;
            }
        }
        print_char(' ', 79, 25, BLACK);
...
char * get_teaterid(char *th_id)
{
    ...
    {
        print_char(' ',79,25,BLACK);
        print_title("Error in opening the file Theaters.bin.bin\n",1,25,RED);
        perror("Reason:1");
        getch();
        return NULL;
    }
...
void cancel_booking_of_user(user *u)
{
    ...
    if(x==0)
    {
        ...
        print_char(' ',79,25,BLACK);
        print_title_in_middle("Booking not cancel!",25,LIGHTGREEN);
        getch();
        return;
    }
    return;
...
int delete_my_Account(user *u)
{
    ...
    else if(choice=='Y'||choice=='y')
    {
            ...
            FILE *fp2=fopen("C:\\Movie Ticket Booking\\Users2.bin","wb");
            if(fp2==NULL)
            {
                print_char(' ',79,25,BLACK);
                print_title("Error in opening the file Bookings.bin\n",1,25,RED);
            ...
            //bookinng
            FILE *fp3=fopen("C:\\Movie Ticket Booking\\Bookings.bin","rb");
            if(fp3==NULL)
            {
                print_char(' ',79,25,BLACK);
                print_title("Error in opening the file Bookings.bin\n",1,25,RED);
            ...
            FILE *fp4=fopen("C:\\Movie Ticket Booking\\TempBookings.bin","wb");
            if(fp4==NULL)
            {
                print_char(' ',79,25,BLACK);
                print_title("Error in opening the file Bookings.bin\n",1,25,RED);
            ...
            if(x==0)
            {
                ...
                if(y==0)
                {
                    ...
                    if(x==0)
                        {
                            y=rename("C:\\Movie Ticket Booking\\Users2.bin","C:\\Movie Ticket Booking\\users.bin");
                            if(y==0)
                            {
                                print_char(' ',79,25,0);
...
int delete_my_account(user *u)
{
    print_basic_screen();
    char choice;
    print_title_in_middle("Are you sure you want to delete your account (Y/y/N/n):",10,YELLOW);
    fflush(stdin);
    scanf("%c",&choice);
    if(choice=='n'|| choice=='N')
    {
